# Pioneer-Follower Model Configuration

## Overview

This configuration file defines the parameters for a **Pioneer-Follower** agent-based simulation model. The model explores how pioneer agents influence the behavior and spatial distribution of follower agents across a grid environment over a series of discrete time steps.
...
|---|---|---|
| `class` | `pioneer_follower` | Model class identifier |
| `model` | `pioneer_follower` | Model name |
...
| `vol_analysis_start` | `20` | Time step at which volatility analysis begins |
| `vol_analysis_end` | `220` | Time step at which volatility analysis ends |
| `num_pioneers` | `10` | Number of pioneer agents |
| `average_response_rate` | `0` | Average rate at which follower agents respond to pioneers |
| `locality_mean` | `0` | Mean of the locality distribution for followers |
| `locality_std` | `1` | Standard deviation of the locality distribution for followers |
| `response_seed` | `123` | Random seed for response rate assignment |
| `locality_seed` | `321` | Random seed for locality assignment |
| `pioneer_flips` | `0` | Number of pioneer behavioral flips |
| `pioneer_locality_mean` | `0` | Mean locality for pioneer agents |
| `pioneer_locality_std` | `0` | Standard deviation of locality for pioneer agents |
...
| `range_response` | `-1, 0` | Range of response rates to sweep over |
...
| Parameter | Value | Description |
|---|---|---|
| `pipe_file` | `configs/pipe.csv` | Path to the pipeline configuration file |
...
> _Provide a brief description of what the Pioneer-Follower model simulates and its research purpose here._
...
- **Locality** — Controls how spatially constrained an agent's awareness or movement is. Higher `locality_std` indicates greater variation in spatial sensitivity.
- **Volatility Window** — Analysis is restricted to steps `vol_analysis_start` through `vol_analysis_end` to exclude initialization and tail effects.
...
- Simulation logs
- Volatility analysis results (steps `20`–`220`)
- Dendrogram clustering results (from `configs/pipe.csv`)
...
> _Add author information, project affiliation, and citation details here._
