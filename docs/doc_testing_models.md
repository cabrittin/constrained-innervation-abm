# Testing Models
When developing new simulation models, users may want to test their model debugging purposes. The scripts/viz\_model.py script provides some boiler plate code for visuallly testing models. Script functionalities are demonstrated below. To use, make sure to replace with the path to your specific model config file. 
...
```
python scripts/viz_model.py viz_grid configs/model.ini 
```
...
```
python scripts/viz_model.py viz_attractor_fields configs/model.ini
```
...
```
python scripts/viz_model.py viz_simulation configs/model.ini
```
